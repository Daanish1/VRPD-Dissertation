...
//Destroy-repair with a set range of destruction (labelled DRR_FR)
public class DestroyRepairFixedRange {
    ...
    public DestroyRepairFixedRange(Random random, SolutionInstance instance, ObjectiveFunction objectiveFunction) {
        this.random = random;
    ...
    //Apply DRR_FR to all truck-drone pairs
    ...
    //Apply DRR_FR to a single truck-drone pair
    private Solution destroyRepairSolution(Solution sol) {
        ...
        //While the time elapsed is less than the max run time allowed repeatedly perform DRR_FR
        while (currentTime - startTime < Constants.RUN_TIME) {
            ...
            int destroyRange = (int) Math.ceil(tempTruckRep.size() * Constants.DESTROY_RANGE_PERCENTAGE);
            ...
            //of temp solution by the smallest amount (Greedy Drone Insertion)
            for (int j = 0; j < reinsertionList.size(); j++) {
            ...
            while (validDroneNodes.size() > 0) {
                ...
                //Assign the landingNode as node after nodeToInsertAsDrone
